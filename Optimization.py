...
# ── BOW Representation (increased features) ────────────────────────────────────
def build_bow(texts):
...
def evaluate_model(name, model, X, y_labels):
    ...
    entry = {
        ...
        "X_tr": X_tr, "X_te": X_te, "y_tr": y_tr, "y_te": y_te, "y_pred": y_pred,
    }
    results.append(entry)
    return entry

# ══════════════════════════════════════════════════════════════════════════════
# BASELINE — 12 ML Models
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("BASELINE EVALUATION — 12 ML Models")
print("="*60)

schemes = [
    ("Scheme1-BOW",   bow1,   MultinomialNB()),
    ("Scheme2-BOW",   bow2,   MultinomialNB()),
    ("Scheme3-BOW",   bow3,   MultinomialNB()),
    ("Scheme1-GloVe", glove1, GaussianNB()),
    ("Scheme2-GloVe", glove2, GaussianNB()),
    ("Scheme3-GloVe", glove3, GaussianNB()),
]

for scheme_name, X, nb_model in schemes:
    evaluate_model(f"NaiveBayes-{scheme_name}", nb_model, X, y)
...
print("\n" + "="*60)
print("FULL SUMMARY — All 18 Models")
print("="*60)
...
# ══════════════════════════════════════════════════════════════════════════════
# OPTIMIZATION — 5-fold CV, bigger grids
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60)
...
scheme_map = {
    "Scheme1-BOW":   (bow1,   True),
    "Scheme2-BOW":   (bow2,   True),
    "Scheme3-BOW":   (bow3,   True),
    "Scheme1-GloVe": (glove1, False),
    "Scheme2-GloVe": (glove2, False),
    "Scheme3-GloVe": (glove3, False),
}

def optimize_model(baseline_entry, X_full, y_full, is_sparse):
    ...
    cv_scores = cross_val_score(best_model, X_full, y_full, cv=cv, scoring="f1_macro")
...
for entry in top3:
    ...
    X_full, is_sparse = scheme_map.get(scheme_key, (bow1, True))
    optimize_model(entry, X_full, y, is_sparse)
...
if not error_df.empty:
    ...
    print("  • Short reviews lack enough tokens for reliable GloVe averaging.")

# ══════════════════════════════════════════════════════════════════════════════
# SAVE BEST MODEL ONLY (for deployment)
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60)
...
_, best_is_sparse = scheme_map.get(best_scheme_key, (glove2, False))
...
print(f"Saved best_model.pkl  →  {best_opt['name']}  (Macro F1={best_opt['macro_f1']})")

summary_data = {
    "baseline":      [{k: v for k, v in r.items() if k not in ("model","X_tr","X_te","y_tr","y_te","y_pred")} for r in results],
